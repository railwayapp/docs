...
const generateIcons = async () => {
  ...
  if (files.length === 0) {
    console.log(`⚠️  No SVG files found in ${colors.red(inputDirRelative)}`);
    return;
  }
...
async function generateSvgSprite(files: string[], outputDirRelative: string) {
  ...
  if (failures.length > 0) {
    console.log(
      `${colors.yellow("⚠️")} Failed to parse: ${failures.join(", ")}`,
    );
  }
  ...
  return writeIfChanged(
    ...
    output,
    `🖼️  Generated SVG spritesheet in ${colors.green(outputDirRelative)}`,
  );
}
