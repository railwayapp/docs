...
Once the cluster is deployed, connect through HAProxy, never to an individual MongoDB node. HAProxy always routes to the current primary, so connections keep working across failovers:
...
After all deployments reach a running state, allow a minute or two for the set to form and for the replicas to complete their initial sync from your original node. The cluster overview in the Railway dashboard shows each node's role, with the current primary carrying a **Primary** badge, along with per-node health.

## Failover

Failover is automatic. When the primary becomes unreachable, the remaining members hold an election. Once a majority agrees, the set elects a new primary and HAProxy reroutes connections to it within one probe interval. In-flight connections to the old primary are dropped; clients that reconnect resume against the new primary without any configuration change, because they connect through HAProxy. When the old primary comes back, it rejoins the set as a secondary and catches up automatically.
