...
<Banner variant="info">
The Tailscale Forwarder template replaces the previous Tailscale Subnet Router template, which is now deprecated. The subnet router advertised the entire <code>fd12::/16</code> private network range as a subnet route. Because every Railway environment uses that same range, only a single subnet router could be used per tailnet, and it required split DNS configuration and manual route approval. The forwarder instead maps explicit ports to services, so you can run one per environment with no route approval and no custom DNS configuration.
</Banner>
...
Since Railway doesn't currently offer a native way to access the <a href="/networking/private-networking" target="_blank">private network</a> from your local environment, you can use a Tailscale Forwarder to accomplish this.
...
- Click **Generate auth key**.
  ...
  The forwarder registers itself as an ephemeral machine, so the key must be reusable for the machine to re-authenticate when the service restarts or redeploys.
...
- A ghost service will appear, asking you to configure the template's variables -
  ...
  - **TS_HOSTNAME** - The machine name the forwarder will register itself under in your tailnet.

    By default this is derived from your project, environment, and service names, e.g. `my-project-production-tailscale-forwarder`. You can leave it as is.
...
This template will start to deploy and once deployed it will register itself as a machine in your tailnet under the configured `TS_HOSTNAME`, you can see it in the <a href="https://login.tailscale.com/admin/machines" target="_blank">Machines dashboard</a>.
